...
// --- Mock API Functions ---
// I've created mock functions so the app is runnable.
// You can replace these with your actual API imports.

const startConversation = async (data) => {
  console.log("Mock API: Starting conversation...", data);
  // Simulate network delay
  await new Promise((res) => setTimeout(res, 300));
  return { id: `conv_${Date.now()}`, title: data.title };
};

const sendMessage = async (data) => {
  console.log("Mock API: Sending message...", data);
  // Simulate network delay for AI response
  await new Promise((res) => setTimeout(res, 1000));
  return {
    ai_message: {
      content: `This is a mock AI response to your message: "${data.content}"`,
    },
  };
};
...
const MessageBubble = ({ sender, content, timestamp }) => {
  ...
  return (
    ...
    >
      {/* 🧠 AI Avatar */}
      {!isUser && (
        <div className="mr-2 flex items-center justify-center rounded-full h-8 w-8 bg-gradient-to-br from-blue-500/60 to-purple-500/40 ring-1 ring-white/10 shadow-lg flex-shrink-0">
      ...
      {/* 💬 Message Bubble */}
      <div
        className={`relative p-3 sm:p-4 max-w-[80%] rounded-2xl backdrop-blur-md shadow-lg transition-all duration-300 ${isUser
          ? "bg-gradient-to-br from-emerald-500/20 to-emerald-300/10 text-white ring-1 ring-emerald-400/20 hover:ring-emerald-400/40"
          : "bg-gradient-to-br from-[#1a1d25] to-[#12141a] text-white/90 ring-1 ring-white/10 hover:ring-white/20"
          }`}
      >
        <p className="whitespace-pre-wrap leading-relaxed text-[15px] sm:text-[16px] tracking-wide">
          {content}
        </p>
        ...
        {timestamp && (
          <p
            className={`absolute text-[10px] opacity-40 bottom-1 ${isUser ? "right-3" : "left-3"
              }`}
          >
      ...
      {/* 👤 User Avatar */}
      {isUser && (
        <div className="ml-2 flex items-center justify-center rounded-full h-8 w-8 bg-gradient-to-br from-emerald-400/60 to-teal-500/40 ring-1 ring-white/10 shadow-lg flex-shrink-0">
          ...
          {!conversation ? (
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto py-4 space-y-4 pr-2">
                ...
                {isLoading && (
                  <div className="flex justify-start items-end my-3 px-2">
                    {/* Updated Loading Avatar */}
                    <div className="mr-2 flex items-center justify-center rounded-full h-8 w-8 bg-gradient-to-br from-blue-500/60 to-purple-500/40 ring-1 ring-white/10 shadow-lg flex-shrink-0">
                      <Bot size={14} className="text-white/80" />
                    </div>
                    {/* Updated Loading Bubble Style */}
                    <div className="relative p-3 sm:p-4 max-w-[80%] rounded-2xl backdrop-blur-md shadow-lg bg-gradient-to-br from-[#1a1d25] to-[#12141a] text-white/90 ring-1 ring-white/10">
                      <motion.div
